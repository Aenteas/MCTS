...
cmake_root = "/home/david/projects/mcts_swig"
build = "".split(" ")
no_build = "".split(" ")
...
def create_cmake_config(lib_dirs):
    ...
    content += 'check_required_components("' + 'mcts_swig")'

    with open(os.path.join(cmake_root, "cmake", "Config.cmake.in"), 'w') as f:
        f.write(content)

    # build info for python
    os.makedirs(os.path.join(cmake_root, "build", "python", "mcts_swig_build"), exist_ok=True)
    with open(os.path.join(cmake_root, "build", "python", "mcts_swig_build", "info.py"), 'w') as f:
        content = ""
        for lib_dir in lib_dirs:
            macro = path_to_macro(lib_dir)
            content += "USE_" + "mcts_swig_" + macro + " = "
