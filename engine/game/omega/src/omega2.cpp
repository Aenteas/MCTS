...
inline Hexagon* Omega2::axToHexP(const vector<vector<Hexagon*>>& board, Ax ax){
...
void Omega2::update(unsigned moveIdx){
    auto[_, pos] = moveIdxToMove(moveIdx);
    ...
    nextPlayer = depth & 2u; // every second turn switch players
    moves.updateNextPiece(nextPiece);
}

// ----- queries ------

inline Hexagon& Omega2::idxToHex(unsigned idx){
    return hexagons[idx];
}
...
const array<double, 2>& Omega2::scores(){
    ...
    while(takenMoves){
        ...
        if(hexagons[pos].mark == mark){ // visited ? taken so should not be empty
            hexagons[pos].mark = !mark;
            ++groupSize;
            queue[end] = &hexagons[pos];
            ++end;
            while(end - start > 0){ // not empty
                auto hex = queue[start];
                for(Hexagon* neighbour : hex->neighbours){
                    if(neighbour->mark == mark && piece == moves[neighbour->idx].piece){ // visited ?, same type of piece as seed ?
...
unsigned Omega2::getNextPiece() const{
    return nextPiece;
}

unsigned Omega2::getLastPiece() const{
    return 1 - nextPiece;
}

const Moves& Omega2::getValidMoves() const{
    return moves.validMoves();
}

const Moves& Omega2::getTakenMoves() const{
    return moves.takenMoves();
}

const Moves& Omega2::getLastMove() const{
