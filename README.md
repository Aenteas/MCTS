...
When we select the current best child node we must hold a lock on it either until we find the new best child or (if it was the best child) until the best grandchild is found. This is because if we release the lock on the currently best child, an other thread might override it during expansion. As a result, selection would proceed from the new leaf node causing incorrect updates. From this we can conclude that we need to hold 3 mutexes at the same time (one for the parent node, one for the currently best child and an other for the currently visited child). Since we need to hold multiple mutextes at the same time we need to prevent deadlocks. This can be achieved by using shared mutexes as proposed or by ensuring the same move ordering for all game states (consistent locking order between threads). The latter has less locking overhead but also less potential for greater concurrecy as it is more likely that the threads would block each other in the selection phase. Preference would depend on various other factors as well like average branching factor and search depth (determining the proportion of exclusive and shared accesses).  
In addition to preventing deadlocks, we need to make sure that parent-child and sibling states are actually different nodes in the tree (it is not necessarily the case when using transposition). Otherwise we potentially lock the same mutex multiple times from the same thread causing an undefined behavior. Assuming that we are using zobrist hashing (XOR) we can do the following: make sure that there is no action hashkey having only zeros (parent-child case) and all the action hashkeys are unique (sibling case). Alternatively, we could extend locking with recursive property. This would entail some performance hit so the first approach is preferred.  
We might want to divide the hash table into blocks of arrays each having a separate mutex to reduce the memory footprint. Unfortunately, we would need recursive property as multiple nodes can be in the same block.  
It turns out that we can just use a single mutex to protect both the node and its corresponding location in the hash table (we also need mutexes in the hash table where there is no node stored).  
